...
- **REQ-OBX-04** — Statuts exposés par entrée : `queued | sending | failed` ; le passage à « envoyé » = sortie de la file (le statut devient celui de la spec 06). Échec définitif (4xx non-ratelimit) → `failed`, renvoi manuel via `retry`, abandon via `remove`.
