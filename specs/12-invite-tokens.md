...
- **REQ-INV-06** — `POST /links/:token/resolve` exige un jeton d'accès Matrix valide et rend `{ kind, issuer }` — plus `roomId` si `kind: group`. **Le service s'arrête là** : aucune invitation n'est émise par lui, et aucun `knock` non plus. Ce que le porteur fait ensuite dépend du `kind` : `friend` → invitation de DM native vers l'émetteur ; `group` → **`knock` sur le salon**, qu'un membre confirme (spec 05 REQ-MSG-20). *(Précisée le 07/08/2026 — escalade E-13. Le service ne gagne aucun droit : la ratification n°1 est intacte, c'est le client qui frappe.)*
...
- **REQ-INV-15** — **Émetteur disparu.** Vérifié à la résolution, jamais mis en cache : un compte désactivé hier ne doit pas rester valide parce qu'il l'était avant-hier. *(Amendée le 05/08/2026, même motif que REQ-INV-14.)* **Le départ de l'émetteur d'un groupe, lui, reste invisible au service** — il n'a aucun droit Matrix. Depuis E-13 la conséquence a changé de nature : le `knock` du porteur atterrit chez **les membres restants**, qui peuvent le confirmer. Un lien dont l'émetteur est parti n'est donc plus une impasse, tant qu'il reste quelqu'un dans le groupe. L'UI d'émission le dit dans ces termes (M-H).
...
- **REQ-INV-16** — **Le service est indisponible.** L'ajout par identifiant Matrix direct reste disponible dans l'UI et **ne passe pas par le service** — il est natif (D-09). Un lien cassé ne doit jamais rendre le produit inutilisable pour se lier à quelqu'un. Critère : le parcours d'ajout par identifiant n'émet aucun appel vers ce service, et **un seul module du dépôt** connaît ses routes — le client de liens du shard (`apps/web/lib/liens-invitation.ts`), gardé par un balayage des sources. *(Amendée le 07/08/2026 — escalade E-13 : le balayage interdisait la route `/resolve` à tout module hors du service, ce qui fermait la porte au client de réception que la voie A exige. La borne devient « un seul appelant nommé » au lieu de « aucun ».)*
...
Suite Vitest, une describe par REQ. Points de contrôle notables : REQ-INV-07 (deux résolutions concurrentes du dernier usage → une seule réussit) ; REQ-INV-08 (les réponses pour un token inconnu, expiré et révoqué sont **strictement identiques**, corps et code) ; REQ-INV-10 à REQ-INV-15 (un test par scénario hors cadre, assertant le comportement **et** l'absence de consommation d'usage là où la spec l'exige) ; REQ-INV-16 (le parcours d'ajout par identifiant n'appelle pas le service ; le balayage des sources ne trouve qu'un seul module connaissant les routes) ; REQ-INV-20 (spy sur le logger : aucun identifiant ni token dans les lignes émises). Un test lisant la configuration asserte qu'aucune variable ne porte de jeton d'administration Synapse.
