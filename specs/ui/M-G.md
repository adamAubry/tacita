...
### Profil
- **REQ-UIX-23** — Profile card (composant 21) : avatar en grand fondant vers le fond (dégradé vers transparence), fond couleur ou photo ; retour en haut à gauche ; en haut à droite : réglages (soi) ou options relatives à la personne (autrui) précédées, pour autrui, du statut ami/non-ami.
- **REQ-UIX-24** — Profil propre : nom + user id juxtaposés avec styles distincts ; Form edit (composant 22, bouton accentué centré) ouvrant le formulaire de modification (nom d'affichage, photo — photo via pipeline, REQ-UI-20) ; navbar avec « Profil » surélevé.
