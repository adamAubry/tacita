...
## E-08 — Le focus RTC est annoncé même quand le SFU est absent (REQ-RTC-05 vs REQ-CAL-02)
`proxy/nginx.conf` publie `org.matrix.msc4143.rtc_foci` dans `.well-known/matrix/client` sans condition — REQ-RTC-05 l'exige, et le test `infra/rtc/tests/network.test.ts` le garde. Mais les backends `/livekit/*` vivent dans l'overlay `rtc/docker-compose.yml`, séparé et exigeant deux IP publiques. Sur toute pile où l'overlay n'est pas monté — dont la pile de développement documentée — `discoverFocus()` trouve un focus valide et l'appel échoue en 502 à la connexion, au lieu du `RtcFociMissing` que REQ-CAL-02 traite en message visible. **Les deux specs sont cohérentes seules ; l'incohérence vit dans la jonction**, mode de panne dominant de ce dépôt. **Proposition** : rendre l'annonce conditionnée à une variable d'environnement (`RTC_ENABLED`), REQ-RTC-05 devenant « expose les `rtc_foci` **quand le RTC est déployé** ». Amendement de spec, donc arbitrage PM — non tranché dans le code. Impact : M-I.
