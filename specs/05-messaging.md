...
> **Réserve portée par REQ-MSG-13, ajoutée le 05/08/2026 avec l'exigence.** L'interdit global « ne jamais trier par `origin_server_ts` » vise l'ordre des messages *dans* une timeline, où /sync fait autorité (REQ-COR-04, REQ-MSG-12) — et il reste entier. Mais une **liste de salons** n'a aucun ordre dans /sync : `getRooms()` rend l'ordre d'insertion du store. La récence du dernier message est le seul signal disponible côté client, et REQ-UIX-07 (tri récentes/anciennes) l'exige. Il est donc utilisé **ici seulement**, jamais à l'intérieur d'un salon. Signalé au PM plutôt que contourné en silence (spec 00, « ce qui ne se décide pas dans le code »).
