...
/**
 * la passerelle push derrière le proxy TLS.
 *
 * Audit des jonctions, entrée n°2. Cette chaîne avait été vérifiée à la main, au curl,
 * en livrant : `infra/tests/push-gateway.test.ts` assère le *contenu des
 * fichiers*, pas le comportement. Par la règle des deux portes, n'avait donc
...
describe("seule la clé publique VAPID sort de la passerelle", () => {
  it("/push/config traverse le proxy TLS et rend la clé du .env", async () => {
    ...
    // `apps/push-gateway/src/server.ts`, pas celui qu'on suppose.
  ...
  it("l'endpoint de notification n'est pas exposé — sinon relais de push ouvert", async () => {
    ...
    // n'a aucune authentification : atteignable de l'extérieur, la
    ...
    // La signature d'une passerelle atteinte, c'est sa réponse, pas un
