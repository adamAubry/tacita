...
- Le **`README.md` de chaque paquet** — son contrat d'interface, ses limites assumées, le motif de ses amendements. C'est là que vit ce qui ne concerne qu'un module.
...
3. **Ne jamais appeler `/search` de Synapse** ni construire de repli dessus (inopérant sur salon chiffré). Recherche 100 % locale.
4. **Pas de libolm** (déprécié) — vodozemac via le SDK.
5. **Ne jamais appeler `/_matrix/media/*/thumbnail`** sur média chiffré — vignettes côté client.
...
9. **Ne pas supposer un accusé « délivré » natif** — Matrix ne définit que `m.read` ; le nôtre est une extension, jamais présentée comme du Matrix natif.
10. **Ne pas décrire `/sync` comme du WebSocket** — c'est du long-polling HTTP.
11. **Pas de canal d'upload parallèle** — un seul pipeline média pour tous les fichiers.
...
Layout officiel du plugin — deux fichiers à la racine, rien dans `.claude/impeccable/` (les anciens `design-system.md`/`interactions.md`/`review-checklist.md` ont été migrés puis supprimés ; leur contenu vit dans DESIGN.md) :
...
- Dev par blocs : un bloc = un lot de comportements cohérent ; commit seulement si les tests du bloc passent. Waterfall jusqu'au ship : les paquets headless se développent en parallèle, `apps/web` s'intègre en dernier ; CI/CD après le ship uniquement.
- **Chaque test nomme le comportement qu'il prouve**, en clair, dans son `describe`. Un nom qui ne dit pas ce qui casserait est rejeté en revue.
- Un module est terminé quand son comportement documenté est couvert par des tests verts et que la suite du paquet passe. Ce n'est pas la même porte que « le produit marche » (règle 4).
...
- **Porte de commit — l'auteur choisit la portée des tests, et l'écrit dans le message.** Hooks bloquants, jamais de `--no-verify`. `typecheck` reste **toujours complet** : c'est lui qui tient les jonctions au niveau des types, et c'est le moins cher des deux. Les tests, eux, se scopent au projet touché **quand le commit est peu impactant**, et couvrent tout le workspace sinon — c'est le défaut, on ne l'obtient pas en le demandant, on le perd en le refusant, et ce refus laisse une trace dans la commande : `TACITA_TESTS="--project @tacita/outbox" git commit -m "…"`. Les noms sont ceux des `package.json`, pas ceux des répertoires : paquets préfixés (`@tacita/outbox`), apps non (`web`, `infra`, `push-gateway`, `invite-tokens`). Mesuré : `@tacita/outbox` 4,6 s contre 85 s pour tout.
  - **Impactant, donc complet** : interface exportée par un paquet, fichier partagé par plusieurs modules, contrat entre deux paquets, composant réutilisé, token ou règle de `DESIGN.md`, configuration de build ou de test.
...
- **Vitest uniquement** ; composants en jsdom/happy-dom, gestes simulés par événements pointer. La config d'`infra` et d'`infra/rtc` est testée aussi : les tests parsent les fichiers YAML rendus et assertent les valeurs critiques.
...
1. **Chaque jonction entre modules a un propriétaire nommé.** Cent pour cent des défauts critiques de ce dépôt étaient des jonctions. Cas d'école : la garde de chiffrement existait dans `messaging` et pas dans `outbox`, parce que le contrat de `messaging` mettait la file hors scope et que celui d'`outbox` ne parlait pas de chiffrement. Les deux contrats respectés, le trou entre eux.
...
4. **« Module terminé » et « produit qui marche » sont deux portes distinctes.** Les tests de configuration attestent le contenu des fichiers ; la cible de fumée atteste un comportement contre un vrai serveur. `infra` a été « 100 % conforme » pendant que personne ne pouvait se connecter, et `invite-tokens` a eu toutes ses cases vertes avant de pouvoir démarrer.
5. **Tenir la promesse ou la retirer — jamais la laisser affichée sans la tenir.** C'est l'interdit n°13. Chaque paquet liste ses limites assumées dans son `README.md` ; elles se documentent, jamais ne se masquent.
...
**Les trois outils ont été évalués le 05/08/2026** — application Next.js 15 réellement construite, rendu éprouvé, registre npm interrogé. Ce qu'il faut en savoir tient en cinq points ; le détail vit dans `apps/web/README.md`.
...
- tout affaiblissement d'une décision de `DECISIONS.md`, en particulier D-08 ;
- toute **contradiction entre deux modules** découverte en les composant. C'est le mode de panne dominant de ce dépôt : chaque module est correct, et l'espace entre eux ne l'est pas.
