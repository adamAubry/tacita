 ...
 * ce que le découpage en worker existe précisément pour éviter.
...
export interface Search {
  /** indexation manuelle ; un événement ou un lot. */
  index(events: IndexableEvent | IndexableEvent[]): Promise<void>;
  /**
   * mot-clé, restreint par les critères fournis (salon, expéditeur,
  ...
  /** taille, plafond et bornes réellement couvertes. */
 ...
 *
 * une édition n'est pas un nouveau message : elle est indexée sous
...
function indexable(event: MatrixEvent): IndexableEvent | undefined {
  ...
  return {
    ...
    body,
    // une correction peut changer le type comme le texte : les deux se
...
/**
 * `m.mentions` est du contenu déchiffré : il ne quitte pas ce chemin, ne
...
/**
 * proxy du worker. Le `Worker` est fourni par l'appelant : ce package
 * ne connaît pas le bundler de l'application.
 *
 * rien n'écoute les rotations de session Megolm. Une rotation ne
...
export function createSearch(session: Session, worker: Worker): Search {
  ...
  // un sync de rattrapage déchiffre en rafale. On accumule sur une
  ...
  /**
   * **l'amorçage : ce que le client tient déjà.**
   *
   ...
   *
   * Le moteur remplace un document déjà connu par son identifiant :
   ...
   * ce que l'écran affiche, et la remontée d'historique fait entrer le reste
  ...
  /**
   * une suppression retire le document. Pas de tampon ici : une redaction
   ...
   * jusqu'au prochain `wipe()`. Brancher une reprise le jour où le shard UI
  ...
  /**
   * l'index est du contenu déchiffré : la déconnexion l'efface. Le tampon
